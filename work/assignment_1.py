...
import env.main_agent as main_agent
import env.ten_arm_env as ten_arm_env
import env.test_env as test_env
...
def argmax(q_values):
    ...
    for i in range(len(q_values)):
        ...
        if q_value > top_value:
            top_value = q_value
            ties = [i]
        # if a value is equal to top value add the index to ties
        elif q_value == top_value:
            ties.append(i)                                    
...
class GreedyAgent(main_agent.Agent):
    def agent_step(self, reward, observation=None):
        ...
        self.arm_count[self.last_action] += 1.0        
        ...
        self.q_values[self.last_action] = new_estimate     
